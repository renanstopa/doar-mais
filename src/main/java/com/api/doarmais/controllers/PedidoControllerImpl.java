...
public class PedidoControllerImpl implements AnuncioController {
  ...
  public ResponseEntity<List<BuscaAnuncioViewModel>> buscar(
    ...
    FiltroAnuncioRequestDto filtro =
        new FiltroAnuncioRequestDto(
            titulo, cidade, tipoUsuario, 2, tipoCategoriaItem, usuarioLogado.getId(), null);
  ...
  public ResponseEntity<PropostaResponseDto> criarProposta(PropostaRequestDto propostaRequestDto) {
    if (!anuncioService.verificarQuantidadeItem(propostaRequestDto))
      throw new ImpossibleItemQuantity(
          "Quantidade escolhida deve ser menor que a restante do item");

    if (!anuncioService.verificarDataAgendada(propostaRequestDto))
      throw new WrongDate(
          "Data da proposta deve estar entre as datas estipuladas pelo criador do anúncio");
    ...
    return new ResponseEntity<PropostaResponseDto>(
        propostaService.gerarResponse(propostaModel), HttpStatus.CREATED);
  }
